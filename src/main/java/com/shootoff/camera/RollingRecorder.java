...
public class RollingRecorder {
	private final Logger logger = LoggerFactory
			.getLogger(RollingRecorder.class);
	...
	public RollingRecorder(ICodec.ID codec, String extension,
			String sessionName, String cameraName, CameraManager cameraManager) {
		this.codec = codec;
		...
		relativeVideoFile = new File(sessionName + File.separator + "rolling"
				+ String.valueOf(System.nanoTime()) + extension);
		videoFile = new File(System.getProperty("shootoff.sessions")
				+ File.separator + relativeVideoFile.getPath());

		videoWriter = ToolFactory.makeWriter(videoFile.getPath());
		videoWriter.addVideoStream(0, 0, codec, recordWidth, recordHeight);

		logger.debug("Started recording new rolling video: {}",
				videoFile.getName());
	}
	...
	public void recordFrame(BufferedImage frame) {
		...
		if (forking) {
			...
			if (timestamp >= ShotRecorder.RECORD_LENGTH * 3) {
				logger.debug("Rolling video file {}, timestamp = {} ms",
						relativeVideoFile.getPath(), timestamp);
				fork(false);
			}
		}
	}
	...
	private ForkContext fork(boolean keepOld) {
		...
		if (!keepOld) {
			relativeVideoFile = new File(sessionName + File.separator
					+ "rolling" + String.valueOf(System.nanoTime()) + extension);
		} else {
			relativeVideoFile = new File(sessionName + File.separator
					+ String.valueOf(System.nanoTime()) + extension);
		}

		File videoFile = new File(System.getProperty("shootoff.sessions")
				+ File.separator + relativeVideoFile.getPath());

		IMediaReader reader = ToolFactory.makeReader(this.videoFile.getPath());
		reader.open();
		long startCutTimestamp = (reader.getContainer().getDuration() / 1000)
				- ShotRecorder.RECORD_LENGTH;
		Cutter cutter = new Cutter(videoFile, codec, startCutTimestamp,
				recordWidth, recordHeight);
		reader.addListener(cutter);

		logger.debug(
				"Forking video file {} to {}, keepOld = {}, start cutting at = {} ms",
				this.relativeVideoFile.getPath(), relativeVideoFile.getPath(),
				keepOld, startCutTimestamp);

		while (reader.readPacket() == null)
			;

		ForkContext context = new ForkContext(relativeVideoFile, videoFile,
				cutter.getLastTimestamp(), cutter.getMediaWriter());

		if (keepOld) {
			// We aren't rolling this file because it got too big,
			// the video is being forked (probably because there was
			// a shot)
			File rollingRelativeVideoFile = new File(sessionName
					+ File.separator + "rolling"
					+ String.valueOf(System.nanoTime()) + extension);
			File rollingVideoFile = new File(
					System.getProperty("shootoff.sessions") + File.separator
							+ rollingRelativeVideoFile.getPath());

			IMediaReader r = ToolFactory.makeReader(this.videoFile.getPath());
			r.open();
			Cutter copy = new Cutter(rollingVideoFile, codec, 0, recordWidth,
					recordHeight);
			r.addListener(copy);
			...
			if (!this.videoFile.delete()) {
				logger.warn(
						"Failed to delete expired rolling video file: {}, keepOld = {}",
						this.videoFile.getPath(), keepOld);
			}
	...
	public ShotRecorder fork() {
		...
		return new ShotRecorder(context.getRelativeVideoFile(),
				context.getVideoFile(), context.getLastTimestamp(),
	...
	private static class ForkContext {
		...
		public ForkContext(File relativeVideoFile, File videoFile,
				long lastTimestamp, IMediaWriter videoWriter) {
			this.relativeVideoFile = relativeVideoFile;
			this.videoFile = videoFile;
	...
	private static class Cutter extends MediaListenerAdapter {
		...
		public Cutter(File newVideoFile, ICodec.ID codec,
				long startingTimestamp /* ms */, int recordWidth,
				int recordHeight) {
		...
		public void onVideoPicture(IVideoPictureEvent event) {
			...
			// seconds of footage
			if (event.getTimeStamp() >= startingTimestamp
					|| startingTimestamp < 0) {
	...
	public void close() {
		...
		if (!videoFile.delete()) {
			logger.warn(
					"Failed to delete expired rolling video file on close: {}",
					videoFile.getPath());
		}
	}
}
