...
public class XMLSessionWriter implements EventVisitor {
	private final Logger logger = LoggerFactory
			.getLogger(XMLSessionWriter.class);
		...
		if (videoString.isPresent()) {
			xmlBody.append(String
					.format(Locale.US,
							...
							timestamp, shot.getColor().toString(), shot.getX(),
							shot.getY(), shot.getTimestamp(), (int) shot
									.getMarker().getRadiusX(), isMalfunction,
							isReload, targIndex, hitRegIndex, videoString.get()));
			...
			xmlBody.append(String
					.format(Locale.US,
							...
							shot.getY(), shot.getTimestamp(), (int) shot
									.getMarker().getRadiusX(), isMalfunction,
							isReload, targIndex, hitRegIndex));
		}
	}
	...
	public void visitTargetAdd(long timestamp, String targetName) {
		xmlBody.append(String.format(
				"\t\t<targetAdded timestamp=\"%d\" name=\"%s\" />%n",
	...
	public void visitTargetRemove(long timestamp, int targetIndex) {
		xmlBody.append(String.format(
				"\t\t<targetRemoved timestamp=\"%d\" index=\"%d\" />%n",
	...
	public void visitTargetResize(long timestamp, int targetIndex,
		...
		xmlBody.append(String.format(Locale.US,
				"\t\t<targetResized timestamp=\"%d\" index=\"%d\" "
						+ "newWidth=\"%f\" newHeight=\"%f\" />%n", timestamp,
				targetIndex, newWidth, newHeight));
	}
	...
	public void visitTargetMove(long timestamp, int targetIndex, int newX,
			int newY) {
		xmlBody.append(String.format(
				"\t\t<targetMoved timestamp=\"%d\" index=\"%d\" "
						+ "newX=\"%d\" newY=\"%d\" />%n", timestamp,
				targetIndex, newX, newY));
	}

	@Override
	public void visitExerciseFeedMessage(long timestamp, String message) {
		xmlBody.append(String
				.format("\t\t<exerciseFeedMessage timestamp=\"%d\">%s%n\t\t</exerciseFeedMessage>%n",
						timestamp, message));
	}
	...
	public void visitEnd() {
		try {
			File sessionsFolder = new File(
					System.getProperty("shootoff.sessions"));
			if (!sessionsFolder.exists()) {
				if (!sessionsFolder.mkdir()) {
					logger.error(
							"Failed to make directory to store sessions: {}",
							sessionsFolder.getPath());
				}
			}
